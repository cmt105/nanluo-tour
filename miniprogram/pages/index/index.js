...
const poiData = [
  ...
  {"name":"雨儿胡同","address":"南锣鼓巷中段西侧","category":"特色胡同","tags":["老北京风风情","静谧"],"desc":"南锣鼓巷西侧最著名的胡同之一。","image":"https://wx4.sinaimg.cn/mw690/006y8mN6ly1g6e2t71v3lj30u00k0779.jpg","latitude":39.935,"longitude":116.401},
...
Page({
  data: {
    // 轮播图数据：根据你的云存储截图，修正后缀为 .jpg 以防报错
    swiperList: [
      { img: "cloud://cloud1-8gv46cfnc1a626b4.636c-cloud1-8gv46cfnc1a626b4-1394447167/poi_001.jpg", text: "南锣鼓巷 · 740年时光" },
      { img: "cloud://cloud1-8gv46cfnc1a626b4.636c-cloud1-8gv46cfnc1a626b4-1394447167/poi_002.jpg", text: "名人故居 · 文化印记" },
      { img: "cloud://cloud1-8gv46cfnc1a626b4.636c-cloud1-8gv46cfnc1a626b4-1394447167/poi_004.jpg", text: "特色胡同 · 烟火人间" }
    ],
    // 今日推荐
    recommendPoi: {
      name: "齐白石旧居纪念馆",
      ...
      image: "cloud://cloud1-8gv46cfnc1a626b4.636c-cloud1-8gv46cfnc1a626b4-1394447167/poi_003.jpg"
    }
  },
  ...
  onLoad: function () {
    // 1. 尝试自动导入数据
    this.autoImportData();
    // 2. 核心：转换首页图片链接，解决 500/403 报错
    this.initHomeImages();
  },

  // 关键补丁：将硬编码的 cloud:// ID 转换为临时的 HTTPS 链接
  initHomeImages: function() {
    // 收集所有需要转换的 ID
    const swiperIds = this.data.swiperList.map(item => item.img);
    const recommendId = this.data.recommendPoi.image;
    const allIds = [...swiperIds, recommendId];

    wx.cloud.getTempFileURL({
      fileList: allIds,
      success: res => {
        // 映射回轮播图
        const newSwiper = this.data.swiperList.map((item, index) => {
          return { ...item, img: res.fileList[index].tempFileURL };
        });
        
        // 映射回推荐位（数组的最后一个元素）
        const newRecommend = { 
          ...this.data.recommendPoi, 
          image: res.fileList[swiperIds.length].tempFileURL 
        };

        // 更新数据，此时图片将正常显示
        this.setData({
          swiperList: newSwiper,
          recommendPoi: newRecommend
        });
      },
      fail: err => {
        console.error("首页图片转换失败", err);
      }
    });
  },

  // 数据库自动检查与导入
  autoImportData: function() {
    console.log('正在检查数据库...')
